...
| REQ-002 | Dependencies are pinned to the versions verified in audit ASM-1. No dependency outside that list is added, and `vite-plugin-pwa` is not installed. **Amended 2026-08-19:** measurement-only devDependencies required by the `critical` reliability gates are admitted — `@vitest/coverage-v8@4.1.11`, `@stryker-mutator/core@10.0.0`, `@stryker-mutator/vitest-runner@10.0.0`. None ships in the bundle. | ASM-1, ASM-4; change owner 2026-08-19 | AC-002 |
| REQ-003 | `tsc --noEmit` passes under TypeScript 6.0.3 in `strict` mode with `noUncheckedIndexedAccess` enabled. | DEC-5, DEC-010; §24 identity rigor | AC-003 |
...
| REQ-072 | Indexes support the queries this change requires: sessions by `status`, by `startedAt` and by `routineId`, exerciseSessions by `sessionId` and by `exerciseId`, completedSets by `exerciseSessionId`, placements by `date` and by `routineId`, workouts by `routineId`, plannedExercises by `workoutId`, routines by `status`. (`sessions.routineId` added 2026-08-19: REQ-075's delete-refusal must ask whether any Session references a Routine, and the original list omitted the index that answers it. Adding it after release would cost a version-2 migration.) | NFR-02; §35; §37 | AC-073 |
...
| DEC-5 | Latest toolchain across the board, pinned per ASM-1, **except TypeScript, which is pinned to `6.0.3`** (amended 2026-08-19, see DEC-010). | Change owner; audit DEC-5 | REQ-002, REQ-003 |
| DEC-010 | **TypeScript is pinned to `6.0.3`, not `7.0.2`.** `typescript-eslint@8.67.0` declares `typescript: ">=4.8.4 <6.1.0"` and throws `typescript-eslint does not support TS 7.0` at load, so REQ-002 and REQ-004 were mutually unsatisfiable under TS 7. Verified directly against the registry and by running `pnpm lint` in the bootstrapped tree on 2026-08-19. Pinning back one major is the alternative audit DEC-5 already enumerated. Vite and Vitest transpile through esbuild/rolldown, so TypeScript only serves `typecheck` and the editor; `strict` and `noUncheckedIndexedAccess` are unaffected. Revisit when typescript-eslint supports TS >= 7.1 (their issue #10940). | Change owner, 2026-08-19, after WS-0 blocked | REQ-002, REQ-003, REQ-004 |
